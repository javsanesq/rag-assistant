...
def _render_report(results: dict, cases: list[BenchmarkCase], top_k: int) -> str:
    ...
    lines.extend(
        [
            ...
            f"- `hybrid_bm25` adds SQLite FTS5 BM25 lexical retrieval. On this corpus it matched dense retrieval's hit rate ({hybrid_summary['hit_rate']:.4f}), which means the answerable questions were not hard enough to expose a BM25 recall gain.",
