...
.PHONY: install test api ui db-upgrade db-revision docker-up docker-config smoke smoke-e2e benchmark docker-down lint
...
benchmark:
	PYTHONPATH=api/src $(PYTHON) scripts/run_benchmark.py
